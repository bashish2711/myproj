...
<html>
   ...
   <a href=/html_show.html target=_new> HTML Gantt Chart </a>
   <a href=/out_show.txt target=_new> Text Log File </a>
   <p> Gantt Chart </p>
   <iframe name="Iframe2" src="/html_show.html" width=100% height=200px> </iframe>
   <p> Run Log </p>
   <iframe name="Iframe1" src="/out_show.txt" width=100% height=200px > </iframe> 
...
class TaskIns(object):
    ...
    def __init__(self, at, end, priority, name, bt):
        self.at    = at
        self.end      = end
        self.usage    = 0
    ...
    for i in xrange(0, hyperperiod):
        for task_type in task_types:
            if  (i - task_type.arrival_time) % task_type.period == 0 and i >= task_type.arrival_time:
                at = i
                end = at + task_type.burst_time
                priority = task_type.deadline - task_type.burst_time -task_type.arrival_time
                tasks.append(TaskIns(at=at, end=end, priority=priority, name=task_type.name, bt=task_type.burst_time))
   ...
   <title> Sucess </title>
    ...
    for i in xrange(0, hyperperiod, clock_step):
        ...
        if len(ready_list) > 0:
            ...
            html += '<td style="background-color:' + html_color[on_cpu.name] + ';">' + on_cpu.get_unique_name() + '</td>' + '\n'
            ...
            # Processor is not used but runtime is going
            run_time += clock_step
            html += '<td style=" text-align: center; width: 20px; height: 50px; background-color:' + html_color['Empty'] + ';">E</td>'  + '\n'
        out += "\n"
    ...
    for run_time in range(0, hyperperiod+1) :
    	html +='<td>' + str(run_time) + '</td>'
       
    # out += Waiting time and turnaround time calculations:
    b_t=[]
    processes=[]
    ...
    #Html output end
    html += "</body></html>"
    html_show = open('../pi/html_show.html', 'w')
    html_show.write(html)
    html_show.close()
    out_show = open('../pi/out_show.txt', 'w')
    out_show.write(out)
    out_show.close()
