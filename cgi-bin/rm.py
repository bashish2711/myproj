...
if __name__ == '__main__':
    ...
    for i in xrange(0, hyperperiod, clock_step):
        ...
        if len(possible) > 0:
            ...
            html += '<div style="float: left; text-align: center; width: 50px; height: 30px; background-color:' + html_color[on_cpu.name] + ';">' + on_cpu.get_unique_name() + '</div>'
            if on_cpu.use(clock_step):
                tasks.remove(on_cpu)
                html += '<div style="float: left; text-align: center; width: 10px; height: 30px; background-color:' + html_color['Finish'] + ';"></div>'
                print "Finish!" ,
        else:
            print 'No task uses the processor. '
            html += '<div style="float: left; text-align: center; width: 5px; height: 30px; background-color:' + html_color['Empty'] + ';">Empty</div>'
        print "\n"
