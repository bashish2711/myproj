...
if __name__ == '__main__':
    ...
    if utilization > 1:
        print '<br /><br />Utilization error!<br /><br />'
    ...
    for i in xrange(0, hyperperiod, clock_step):
        ...
        if len(possible) > 0:
            on_cpu = possible[0]
            print '<div style="float: left; text-align: center; width: 110px; height: 20px; background-color:' + html_color[on_cpu.name] + ';">' + on_cpu.get_unique_name() + '</div>'            
            if on_cpu.use(clock_step):
                tasks.remove(on_cpu)
                print '<div style="float: left; text-align: center; width: 10px; height: 20px; background-color:' + html_color['Finish'] + ';"></div>'
        else:
            print '<div style="float: left; text-align: center; width: 110px; height: 20px; background-color:' + html_color['Empty'] + ';">Empty</div>'
